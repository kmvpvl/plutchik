...
export default class Stats extends React.Component<IStatsProps, IStatsState> {
    ...
    private prepareData(){
        ...
        //Average vectors by item
        this.avgVectorsByItems = [];
        stats.assessments.forEach((a: any)=>{
            const foundA = this.avgVectorsByItems.findIndex(el=>el.cid === a.cid);
            if (foundA !== -1) {
                const newV = new EmotionalVector(a.vector);
                const oldV = new EmotionalVector(this.avgVectorsByItems[foundA].vector);
                oldV.mult(this.avgVectorsByItems[foundA].count);
                oldV.add(newV);
                oldV.mult(1 / (this.avgVectorsByItems[foundA].count + 1));
                this.avgVectorsByItems[foundA].count += 1;
                this.avgVectorsByItems[foundA].vector = oldV as IEmotionalVector;
    ...
    render(): ReactNode {
        return <div className="stats-container">
            ...
            <div className="stats-filters-bar"></div>
