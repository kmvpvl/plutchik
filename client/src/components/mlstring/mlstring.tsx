...
export class MLStringEditor extends React.Component<IMLStringEditorProps, IMLStringEditorState> {
    ...
    render(): React.ReactNode {
        return <span className='mlstring-container' key={Math.random()}>
            ...
            }}>[+]</button><input placeholder="Type in default language" key={`${this.r}_default`} onChange={(e)=>{
            ...
            <span >
                ...
                <input placeholder='Type language' key={`${this.r}_k_lang`} onChange={(e)=>{
                    const oldv = this.state.value.values.get(l);
                    this.state.value.values.delete(l);
                    this.state.value.values.set(e.currentTarget.value, oldv as string);
                    if (this.props.onChange) this.props.onChange(this.value);
                    this.setState(this.state);
                }} defaultValue={l}/><input placeholder='Type here value in language' key={`${this.r}_k_val`} onChange={(e)=>{
